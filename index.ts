...
serve(async (req) => {
  ...
  if (!files["/index.html"]) {
    vercelFiles.push({
      file: "index.html",
      data: `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${projectName}</title><script src="https://cdn.tailwindcss.com"></script></head><body><div id="root"></div><script type="module" src="/App.tsx"></script></body></html>`,
    });
  }
  ...
  const subdomainUrl = `https://${slug}.sitegeniebuild.com`;
  ...
  return new Response(JSON.stringify({ url: subdomainUrl, deployUrl, slug }), {
